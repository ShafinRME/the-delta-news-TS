...
const SingleCrimeNews = () => {
  ...
  const [crimeNews, setCrimeNews] = useState<NewsProps[]>([]); // this state for all crime news
  const [singleCrimeNews, setSingleCrimeNews] = useState<
    SingleNewsProps | undefined
  >(undefined); //this state for singleCrimeNews

  //this is for singleCrimeNews
  useEffect(() => {
    // fetch(`${url}/crime`)
    fetch(`https://the-delta-times-server.vercel.app/api/news/crime/${slug}`)
      .then((res) => res.json())
      .then((data) => setSingleCrimeNews(data));
  }, [slug]);

  //this is for all crime
  useEffect(() => {
    // fetch(`${url}/crime`)
    fetch(`https://the-delta-times-server.vercel.app/api/news/crime`)
      .then((res) => res.json())
      .then((data) => setCrimeNews(data));
  }, []);
  return (
    <>
          ...
          <section>
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
              <div className="xl:col-span-8">
                ...
                <div className="p-2 border">
                  <h1 className="news-sub-title-three-col m-2">
                    {singleCrimeNews?.title}
                  </h1>
                  <img className="w-full" src={singleCrimeNews?.image} alt="" />
                  <p className="">{singleCrimeNews?.description}</p>
                </div>
              </div>
              {/* right side */}
              <div className="xl:col-span-4">
                ...
                {crimeNews.slice(11, 16).map((news) => (
                  <>
                    <Link to={`/bangladesh/crime/${news.slug}`}>
                      <div
