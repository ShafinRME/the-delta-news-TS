...
const SinglePoliticsNews = () => {
  ...
  const [politicsNews, setPoliticsNews] = useState<NewsProps[]>([]); // this state for all politics news
  const [singlePoliticsNews, setSinglePoliticsNews] = useState<
    SingleNewsProps | undefined
  >(undefined); //this state for singlePoliticsNews

  //this is for singlePoliticsNews
  useEffect(() => {
    // fetch(`${url}/politics`)
    fetch(`https://the-delta-times-server.vercel.app/api/news/politics/${slug}`)
      .then((res) => res.json())
      .then((data) => setSinglePoliticsNews(data));
  }, [slug]);

  //this is for all politics
  useEffect(() => {
    // fetch(`${url}/politics`)
    fetch(`https://the-delta-times-server.vercel.app/api/news/politics`)
      .then((res) => res.json())
          ...
          <section>
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
              <div className="xl:col-span-8">
                ...
                <div className="p-2 border">
                  <h1 className="news-sub-title-three-col m-2">
                    {singlePoliticsNews?.title}
                  </h1>
                  <img
                    className="w-full"
                    src={singlePoliticsNews?.image}
                    alt=""
                  />
                  <p className="">{singlePoliticsNews?.description}</p>
                </div>
              </div>
              {/* right side */}
              <div className="xl:col-span-4">
                ...
                {politicsNews.slice(11, 16).map((news) => (
                  <>
                    <Link to={`/bangladesh/politics/${news.slug}`}>
                      <div
