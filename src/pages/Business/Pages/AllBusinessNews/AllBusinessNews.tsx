...
const AllBusinessNews = () => {
  const [businessNews, setBusinessNews] = useState<NewsProps[]>([]);
  useEffect(() => {
    fetch(`${url}/Business`)
      .then((res) => res.json())
      .then((data) => setBusinessNews(data));
  }, []);
  return (
    <>
        ...
        <>
          ...
          <section>
            <div className="grid grid-cols-1 md:grid-cols-1 xl:grid-cols-12 gap-4 border-b border-warning-content pb-4">
              ...
              <div className="xl:col-span-9">
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 pb-4 border-b border-warning-content">
                  <div className="xl:col-span-8">
                    {businessNews.slice(0, 1).map((item) => (
                      <div key={item.id}>
                        <Link to={`${item.slug}`}>
                          <img
                            src={item.image}
                  ...
                  <div className="xl:col-span-4 xl:border-l   border-warning-content xl:pl-4">
                    {businessNews.slice(1, 2).map((item) => (
                      <div key={item.id}>
                        <Link to={`${item.slug}`}>
                          <img
                            src={item.image}
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 pt-4 gap-4 ">
                  {businessNews.slice(3, 6).map((item) => (
                    <div
                      key={item.id}
              ...
              <div className="xl:col-span-3 xl:pl-4 xl:border-l xl:border-warning-content">
                ...
                {businessNews.slice(6, 10).map((item) => (
                  <div
                    key={item.id}
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 xl:grid-cols-12 gap-4  pb-4 pt-14">
              {/* left part */}
              <div className="xl:col-span-9">
                {businessNews.slice(10).map((item) => (
                  <div
                    key={item.id}
