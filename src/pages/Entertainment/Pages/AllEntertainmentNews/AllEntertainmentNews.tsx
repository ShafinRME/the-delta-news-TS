...
const AllEntertainmentNews = () => {
  const [allEntertainmentNews, setAllEntertainmentNews] = useState<NewsProps[]>(
    []
  );
  useEffect(() => {
    fetch(`${url}/Entertainment`)
      .then((res) => res.json())
      .then((data) => setAllEntertainmentNews(data));
  }, []);
  return (
    <>
        ...
        <>
          ...
          <section>
            <div className="grid grid-cols-1 md:grid-cols-1 xl:grid-cols-12 gap-4 border-b border-warning-content pb-4">
              ...
              <div className="xl:col-span-9">
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 pb-4 border-b border-warning-content">
                  <div className="xl:col-span-8">
                    {allEntertainmentNews.slice(0, 1).map((item) => (
                      <div key={item.id}>
                        <Link to={`${item.slug}`}>
                          <img
                            src={item.image}
                  ...
                  <div className="xl:col-span-4 xl:border-l   border-warning-content xl:pl-4">
                    {allEntertainmentNews.slice(1, 2).map((item) => (
                      <div key={item.id}>
                        <Link to={`${item.slug}`}>
                          <img
                            src={item.image}
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 pt-4 gap-4 ">
                  {allEntertainmentNews.slice(3, 6).map((item) => (
                    <div
                      key={item.id}
              ...
              <div className="xl:col-span-3 xl:pl-4 xl:border-l xl:border-warning-content">
                ...
                {allEntertainmentNews.slice(6, 10).map((item) => (
                  <div
                    key={item.id}
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 xl:grid-cols-12 gap-4  pb-4 pt-14">
              {/* left part */}
              <div className="xl:col-span-9">
                {allEntertainmentNews.slice(10).map((item) => (
                  <div
                    key={item.id}
