...
const SingleAsiaNews = () => {
  ...
  const [asiaNews, setAsiaNews] = useState<NewsProps[]>([]); // this state for all america news
  const [singleAsiaNews, setSingleAsiaNews] = useState<
  ...
  useEffect(() => {
    // fetch(`${url}/america`)
    fetch(`https://the-delta-times-server.vercel.app/api/news/asia/${slug}`)
      .then((res) => res.json())
      .then((data) => setSingleAsiaNews(data));
  }, [slug]);
            ...
            <div className="xl:col-span-8">
              ...
              <div className="p-2 border">
                <h1 className="news-sub-title-three-col m-2">
                  {singleAsiaNews?.title}
                </h1>
                <img className="w-full" src={singleAsiaNews?.image} alt="" />
                <p className="">{singleAsiaNews?.description}</p>
              </div>
            </div>
            {/* right side */}
